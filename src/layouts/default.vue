<template>
  <div id="app">
    <div style="display:none;"
         v-for="item in this.$store.state.seoBlog"
         :key="item.id">
...
<script>
import { GetWebConfig, GetAllBlog } from "@/api/server";
export default {
  name: "app",
  data () {
    return {};
  ...
  head () {
    return {
      title: this.$store.state.webConfig.basics.blogName + (this.$store.state.webConfig.basics.blogInfo ? ' - ' + this.$store.state.webConfig.basics.blogInfo : ''),
      meta: [
      ...
      link: [
        { rel: 'icon', type: 'image/x-icon', href: this.$store.state.webConfig.basics.blogIcon || '/img/favicon.ico' }
      ]
    }
  },
  async fetch () {
    ...
    let seoblogRes = await GetAllBlog();
    this.$store.commit('setSeoBlog', seoblogRes.list);
