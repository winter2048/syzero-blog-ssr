...
<template>
  <div class="rightlistBox">
    <section>
      <div class="r1-head">
        <img :src="
            this.$store.state.webConfig.other.otherRightImg
              ? this.$store.state.webConfig.other.otherRightImg
              : ''
          "
             alt="" />
        <h1 v-if="this.$store.state.themeObj.user_start != 0">
          {{ this.$store.state.webConfig.user.nickName }}
        </h1>
      </div>
      <div class="r1-body">
        <p>你能抓到我么？</p>
        <div class="catch-me">
          <div class="">
            <el-tooltip class="item"
                        content="Github"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherGithub"
                 v-if="this.$store.state.webConfig.other.otherGithub"
            ...
            <el-tooltip class="item"
                        ...
                        content="QQ"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherQQ"
                 v-if="this.$store.state.webConfig.other.otherQQ"
                 target="_blank"><i class="fa fa-fw fa-qq"></i></a>
            </el-tooltip>
            <el-tooltip class="item"
                        effect="dark"
                        content="微博"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherWeibo"
                 v-if="this.$store.state.webConfig.other.otherWeibo"
          ...
          <div class="">
            <el-tooltip class="item"
                        ...
                        content="微信"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherWx"
                 v-if="this.$store.state.webConfig.other.otherWx"
                 target="_blank"><i class="fa fa-fw fa-wechat"></i></a>
            </el-tooltip>
            <el-tooltip class="item"
                        effect="dark"
                        content="CSDN"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherCsdn"
                 v-if="this.$store.state.webConfig.other.otherCsdn"
            ...
            <el-tooltip class="item"
                        ...
                        content="简历"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherJianli"
                 v-if="this.$store.state.webConfig.other.otherJianli"
                 target="_blank"><i class="fa fa-fw fa-file-word-o"></i></a>
            </el-tooltip>
            <el-tooltip class="item"
                        effect="dark"
                        content="更多"
                        placement="top">
              <a :href="this.$store.state.webConfig.other.otherMore"
                 v-if="this.$store.state.webConfig.other.otherMore"><i class="el-icon-more"></i></a>
            </el-tooltip>
          </div>
        </div>
      </div>
...
import { GetBlog, AddLikeNum, ShowLikeNum } from "../api/server.js";
...
export default {
  ...
  async mounted () {
    ...
    //查询浏览量最多的10篇文章数据
    let dataRes = await GetBlog({ Sort: "viewNums desc", Limit: 10 });
    that.browseList = dataRes.list;

    let likeRes = await ShowLikeNum();
    that.likeNum = that.initLikeNum = likeRes;
    // //查询文章评论量最大的10篇文章
